    ...
    init() {
        return new Promise((resolve, reject) => {
            if (suggestionCooldown.has(message.author.id)) {
                this.message.delete();
                this.message.author.send(`Your recent message in #suggestions has been removed as you are on cooldown. The cooldown lasts 30 seconds. Here is your suggestion in case you need it:\n \`\`\`${this.message.content}\`\`\``);
                return resolve();
            } else {
                suggestionCooldown.add(message.author.id);
                this.message.channel.send(new client.modules.Discord.MessageEmbed()
    ...
    init() {
        return new Promise((resolve, reject) => {
            this.client.models.supportTickets.findOne({
                ...
                if (err) return reject(err);
                if (!db) return reject(`User typing in channel found in SUPPORT TICKETS but no SUPPORT TICKET DATABASE ENTRY can be found! (${this.message.channel.name} ${this.message.channel.id})`)
                if (db.options.find(x => x.name == "ticket_mentioning").boolean == true) {
                    if (this.message.author.id == db.user_id) return resolve();
                    if (this.message.mentions.users.first().id == db.user_id) return resolve();
                    this.message.channel.send(`<@${db.user_id}>`).then(msg => setTimeout(() => msg.delete(), 10));
                }
                db.logs.push({
        ...
        return new Promise((resolve, reject) => {
            let obj = {
                'xp_ping': this.client.models.userSettings.findOne({
                    ...
                    if (err) return reject(err);
                    return db.options.find(x => x.name == 'xp_ping').boolean == true ? resolve(true) : resolve(false);
                }),
                'coin_ping': this.client.models.userSettings.findOne({
                    "user_id": this.message.author.id
                }, (err, db) => {
                    if (err) return reject(err);
...
class handledb extends dbfunctions {
    ...
    async run(client, message) {
        this.addXP(client, message).catch(err => new client.methods.log(client).error(err));
        this.checkLevel(client, message).catch(err => new client.methods.log(client).error(err));
        this.addCoins(client, message).catch(err => new client.methods.log(client).error(err));
        this.addMessageCount(client, message).catch(err => new client.methods.log(client).error(err));
    }
            ...
            client.models.userProfiles.findOne({
                ...
                if (await this.checkXPtoLevel(db.user_level, db.user_xp)) {
                    message.channel.send(new client.modules.Discord.MessageEmbed()
                        ...
                        .setDescription(`<@${message.author.id}>! You have reached **${db.user_level * 1000} XP** and have ranked up to **Level ${db.user_level}**!`)
                    );
                    db.user_level += 1;
                    db.save((err) => {
                        if (err) return reject(err);
            ...
            client.models.userProfiles.findOne({
                ...
                if (client.functions.percentChance(35)) {
                    ...
                    if (await this.checkSettings('coin_ping')) {
                        message.channel.send(`<@${message.author.id}> **+${coin} Coins**`).then(msg => setTimeout(() => msg.delete(), 1800));
                    }
                    db.save((err) => {
                        if (err) return reject(err);
                        else return resolve();
...
module.exports = (client, message) => {
    ...
    if (message.content.toString().startsWith("-")) {
        ...
        try {
            ...
            return new client.methods.log(client, message.guild).commandRan(message.author, cmd.name, message.channel.id);
        } catch (err) {
            return new client.methods.log(client).error(err);
        }
    } else {
        if (message.channel.name == "suggestions") {
            return new suggestion(client, message).init().catch(err => new client.methods.log(client).error(err));
        }
        if (message.channel.parentID == "590285807265251339") {
            new ticket(client, message).init().catch(err => new client.methods.log(client).error(err));
        }
